...
/*! \file  time_advance_RBslipMHD.cc
 *  
...
//
	...
	{
		Compute_rhs(W, T, Pr_switch);                                          
		Single_time_step_EULER(W, T, Pr_switch, Tdt);     
	}
...
//
	...
	{	
		...
		// Vcopy[i] <- V[i] ; Scopy <- T.F 

		Compute_rhs(W, T, Pr_switch);  
  
		Single_time_step_EULER(W, T, Pr_switch, Tdt/2);				
		// Goto the mid point
  
		Compute_force_RB(W, T, Ra, Pr, Pr_switch, RB_Uscaling);
...
//		Satisfy_reality_condition_nlin(W, T);
		...
		Compute_pressure();                        
		Compute_rhs(W, T,  Pr_switch);								
		// compute rhs using V,T(t+dt/2) 
  
		Copy_field_from(Vcopy); W.Copy_field_from(Wcopy);
		T.Copy_field_from(Scopy);
	 
		IncFluid::Single_time_step_RK2(W, T, Pr_switch, Tdt);		
...
//
	...
	else if (integ_scheme == "RK4") 
	{
		...
		Compute_rhs(W, T, Pr_switch);    
		Single_time_step_EULER(W, T, Pr_switch, Tdt/2);			// u1: Goto the mid point
		...
		Compute_force_RB(W, T, Ra, Pr, Pr_switch, RB_Uscaling);
		Compute_nlin(W, T, Pr_switch);							// Compute nlin using V(t+dt/2)
		Satisfy_reality_condition_nlin(W, T);
		Add_force(W, T, Pr_switch);								// nlin = nlin - f
		Compute_pressure();										// Compute pressure using V(t+dt/2)
		Compute_rhs(W, T, Pr_switch); 
		Copy_field_from(Vcopy);	W.Copy_field_from(Wcopy); 
		T.Copy_field_from(Scopy);
		Single_time_step_Semi_implicit(W, T, Pr_switch, Tdt/2);	// u2: Goto the mid point
		...
		Copy_field_from(Vcopy); W.Copy_field_from(Wcopy); 
		T.Copy_field_from(Scopy);
		Single_time_step_RK2(W, T, Pr_switch, Tdt);				// u3: Go to the end point
