...
/*! @brief 3D: Compute \f$ (\Delta v)^q \f$ for a pair of points
 * 
 *	@param	TinyVector<int,2> j1, j2 The two point.
 *	@param TinyVector<DP,2> V1, V2 Velocity at the two points.
 ...
 *  @return \f$ St(r,q,0) += |\Delta v_{\perp}|^q \f$.
 *	@return	count(r) += 1.
 *
 */
...
/// Same as above for scalar F in 3D.  
/// Returns \f$ St(r,q) += |\Delta F|^q \f$  with r=ceil(|r2-r1|).
void Compute_dSt
(
	string basis_type,
	int N[],
...
(
	...
	int structurefn_q_min, int structurefn_q_max, 
	Array<DP,3> St, 
	DP xfactor[]
);
