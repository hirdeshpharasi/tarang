...
)
{
	...
	ArraySFT_SCFT_transpose_order(sintr_plan_SCFT, r2c_plan_SCFT, N, temp_r, A);
}
...
{
	Transpose_array(N, A, temp_r);
	ArrayCFT_SCFT_transpose_order(costr_plan_SCFT, r2c_plan_SCFT, N, temp_r, A);
}									
...
{
	ArrayISFT_SCFT_transpose_order(isintr_plan_SCFT, c2r_plan_SCFT, N, A, temp_r);
...
{
	ArrayICFT_SCFT_transpose_order(icostr_plan_SCFT, c2r_plan_SCFT, N, A, temp_r);
	Inverse_transpose_array(N, temp_r, A);
}
 ...
 )  
{
	...
	for (l1=0; l1 < local_N1; l1++)   								// FFT along y
	{
		vertical_col = A(l1, Range::all());
		IFT_vertical_SCFT(c2r_plan_SCFT, N, vertical_col);
		A(l1, Range::all()) = vertical_col;
	}
