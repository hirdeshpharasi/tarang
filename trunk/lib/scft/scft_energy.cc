...
{	
	// m != 0, ky = ny plane
	DP tot_plane =  real( 2.0*sum(A(Range::all(),ny,Range::all()) 
...
)
{
	...
	DP entropy;
	MPI_Reduce(&local_entropy, &entropy, 1, MPI_DOUBLE, MPI_SUM, master_id, MPI_COMM_WORLD);	
...
)
{
	...
	Compute_local_ring_spectrum_SCFT(alias_switch, N, F, cylinder_kpll_array, n, 
										local_Sk, kfactor);
...
)
{
	...
	Compute_local_ring_spectrum_SCFT(alias_switch, N, F, G, cylinder_kpll_array, n, 
										local_Sk, kfactor);
...
)
{
	...
	Compute_local_ring_spectrum_helicity_SCFT(alias_switch, N, Ax, Ay, Az, cylinder_kpll_array, 
												local_H1k, kfactor);
