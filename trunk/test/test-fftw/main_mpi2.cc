...
int main(int argc, char **argv)
{
    ...
    Array<complex<double>,3>  *B;
		...
		NN[1] = N0; NN[2] = N1; NN[3] = N2;
         ...
         alloc_local = fftw_mpi_local_size_3d_transposed(N0, N1, N2, MPI_COMM_WORLD,
   ...
   //      data = (fftw_complex *) fftw_malloc(sizeof(fftw_complex) * alloc_local);
	  ...
	  r2c_plan_FOUR = fftw_mpi_plan_transpose(NN[1], NN[2], reinterpret_cast<double*>((*A).data()), 
			     reinterpret_cast<fftw_complex*>((*A).data()),  MPI_COMM_WORLD, 
				 FFTW_MPI_TRANSPOSED_IN);
	   c2r_plan_FOUR = fftw_mpi_plan_transpose(NN[1], NN[2], reinterpret_cast<fftw_complex*>((*A).data()), 
			     reinterpret_cast<double*>((*A).data()), MPI_COMM_WORLD,
